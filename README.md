...
**Super-admin** credentials are set via `SUPER_ADMIN_EMAIL` / `SUPER_ADMIN_PASSWORD` environment variables and are always kept with `ADMIN` role on every application boot via `BootstrapService.ensureSuperAdmin()`.
